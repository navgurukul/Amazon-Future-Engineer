...
const MegaPage: NextPage = () => {
  ...
  const [megaProgram, setMegaProgram] = useState<any>(null);

  useEffect(() => {
    const programData = JSON.parse(localStorage.getItem("programData") || "[]");
    const foundMegaProgram = programData.find(
      (program: any) => program.title === "MEGA"
    );
  ...
  const validateForm = () => {
    ...
    if (formData.name.trim() === "" || !isNaN(Number(formData.name))) {
      newErrors.name =
        "Please enter a valid name. It cannot be empty or a number.";
    }
    ...
    if (formData.schoolName.trim() === "" || /\d/.test(formData.schoolName)) {
      newErrors.schoolName =
        "Please enter a valid school name. It cannot be empty or contain numbers.";
    }
